      ...
      <header className={`heading ${scrolled ? 'position-fixed' : ''}`}>
        <Container>
          <div className='heading-wrap flex items-center justify-between'>
              ...
              <ul className="lg:flex items-center gap-2">
                {navLinks.map((link, index) => (
                  <li key={index} className='d-block'>
                    <Link className="heading-link capitalize" to={link.to} spy={true} smooth={true} duration={600} offset={0} onClick={() => handleTouchStart(link.to)}>{link.btn_title}</Link>
                  </li>
                ))}
            ...
            <Social className="hidden xl:flex"/>
            <div className="heading-actions flex items-center flex-wrap">
              <a href='#' className='btn uppercase text-white tracking-[2.56px] leading-[80%]'>Buy henzo</a>
